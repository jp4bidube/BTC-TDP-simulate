...
export default async function BitcoinService(value, date) {
  ...
  data = await api.get('https://api.coindesk.com/v1/bpi/historical/close.json', {
    params: {
  ...
  let USD = await api.get("https://economia.awesomeapi.com.br/all/USD-BRL")
    .then((response) => { return response.data.USD.ask })
